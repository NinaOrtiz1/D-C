...
export async function getDashboardStats(_req: Request, res: Response) {
  try {
    ...
    const [totalUsers, totalProducts, totalCategories, totalComments, totalContacts, recentLogins, lowStockProducts] = await Promise.all([
      ...
      Product.find({ activo: true, stock: { $lte: 5 } })
        .limit(10)
